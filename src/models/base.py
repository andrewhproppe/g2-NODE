...
from src.pipeline.data import H5Dataset
from src.pipeline.transforms import flatten_time_sequence
from src.models.utils import BetaRateScheduler
...
def fourier_loss(y, yhat, indices=None):
    """
    - (1 - image) converts g2 into interferogram, which is Fourier transformed along the δ axis
    ...
    y = torch.fft.fft((1 - y), dim=1)
    yhat = torch.fft.fft((1 - yhat), dim=1)
    #
    # y = torch.fft.fftshift(y, dim=1)
    # yhat = torch.fft.fftshift(yhat, dim=1)
    # #
    # y = torch.fft.fftshift(torch.fft.fft(y, dim=1), dim=1)
    # yhat = torch.fft.fftshift(torch.fft.fft(yhat, dim=1), dim=1)

    loss = F.mse_loss(y.real, yhat.real)
...
class MonotonicLoss(nn.Module):
    """
    Penalizes violations of monotonicity along the specified dimension.
    For PCFS, dimension must be equal to 2 to ensure monotonically increasing correlations. If dimension 1 is used,
    will instead penalize oscillations, which is not desired.
    """

    def __init__(self, dimension=2, weight=1.0):
        super(MonotonicLoss, self).__init__()
        self.dimension = dimension
        self.weight = weight

    def forward(self, x):
        # Calculate the differences along the specified dimension
        diffs = torch.diff(x, dim=self.dimension)

        # Ensure that the differences are non-negative (monotonic increasing)
        violation_mask = (diffs < 0).float()

        # Calculate the loss as the sum of violations along the specified dimension
        # loss = torch.sum(violation_mask, dim=(1, 2))
        loss = torch.sum(violation_mask)

        # Normalize the loss to be between 0 and 1
        normalized_loss = loss / violation_mask.numel()

        # Apply the weight
        normalized_loss = self.weight * normalized_loss
...
class Conv2DAutoEncoder(AutoEncoder):
    def __init__(
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(
                channels[2], channels[1], kernel1, stride=2, padding=1, output_padding=1
            ),
            nn.ConvTranspose2d(
                channels[1], channels[0], kernel1, stride=2, padding=1, output_padding=1
            ),
            # nn.Sigmoid()
        )
...
class Conv1DAutoEncoderGold(AutoEncoder):
    def __init__(
        ...
        flat_shape = get_conv1d_flat_shape(
            self.encoder, torch.ones(input_dim)
        )  # for linear bottleneck
        conv_shape = get_conv_output_shape(
            encoder_conv, torch.ones(input_dim)
        )  # for Reshape layer
...
class AdversarialEnsemble2(EnsembleSubmodels):
    ...
    def step(self, batch, batch_idx: int, optimizer_idx: int, prefix: str):
        ...
        if optimizer_idx == 0:
            ...
            self.log(f"{prefix}", logs, on_epoch=True, on_step=False)
            loss = (
                submodel_loss + ensemble_loss + encoding_loss * self.hparams.enc_weight
            )
...
if __name__ == "__main__":
    ...
    X = dm.train_set.dataset.g2s[0:5]

    y = torch.tensor(1 - X)
    # test_image = torch.tensor(X[0])
    # y = y[:, :, 0]
    y_fft =  torch.fft.fftshift(torch.fft.fft(y, dim=1), dim=1)
    # y_fft_shifted = torch.fft.fftshift(y_fft, dim=1)
    # plt.plot(y_fft[0])
    plt.imshow(y_fft[0].real)

    # def fourier_loss(input_image, target_image, index=None):
    #     """
    #     - (1 - image) converts g2 into interferogram, which is Fourier transformed along the δ axis
    #     - Adapted to just look at interferogram for τ = 0, where oscillations are the strongest
    #     """
    ...
    #
    #     y_fft = torch.fft.fftshift(torch.fft.fft(y, dim=1), dim=1)
    #     y_hat_fft = torch.fft.fftshift(torch.fft.fft(y_hat, dim=1), dim=1)
    #
