...
from src.modules.PCFS import (
    energy_to_ζ,
    spec_corr_to_interferogram_FFT,
    fourier_ζ_to_ps,
    threeLorentzian_oneFWHM,
    make_theta_threeLorentzian_oneFWHM,
    twoLorentzian_oneJ,
    make_theta_twoLorentzian_oneJ,
)
import pickle
...
def gauss_peak(x, x0, gauss_fwhm):
    return (gauss_fwhm / (2 * np.sqrt(2 * np.log(2)))) * np.exp(
        -(4 * np.log(2) / (gauss_fwhm**2)) * (x - x0) ** 2
    )
...
def compare_g2s(
    ...
    if len(delays) > 1:
        fig, axes = plt.subplots(
            figsize=(4 * Cols, 3 * Rows), dpi=150, nrows=Rows, ncols=Cols, squeeze=False
        )
        axes = axes.flatten()
        for i, d in enumerate(delays):
            ...
            if show_g2:
                axes[i].semilogx(
                    sdot.τ, dot.g2[:, idx], label="Data (%.1f ps)" % d, color=colors[i]
                )
            if show_xcorr:
                axes[i].semilogx(
                    sdot.τ, dot.cross_correlations[idx], color=colors[i], label="Data"
                )
            if show_acorr:
                axes[i].semilogx(
                    sdot.τ, dot.auto_correlations[idx], color=colors[i], label="Data"
                )
            axes[i].semilogx(
                sdot.τ, sdot.g2[:, idx], label="Simulated", color=colors[i] * 0.75
            )
...
class SimulatedPCFS(object):
    def __init__(
        ...
        if dot is not None:
            ...
            self.dot_delay = (
                dot.optical_delay
            )  # need the actual stage positions if nonlinear stepping was used
        ...
        color = np.array(sns.color_palette("icefire", ncolors + 2))
    ...
    def make_delay_nonlinear(self, nonlinear_args):
        e = (
            np.logspace(
                start=nonlinear_args[0],
                stop=nonlinear_args[1],
                base=nonlinear_args[2],
                num=self.nstage,
                endpoint=True,
            )
            - 1
    ...
    def make_spectrum(
        self, weg, lineshape, theta, nbins=501, emax=5, norm=True, zero_baseline=False
    ):
        """
        ...
        self.i = int(
            len(self.δ) / 2
        )  # an index to find the white fringe if using symmetric interferograms
        # self.ζ = self.ζ
        ...
        self.homogeneous_spec_corr = np.correlate(
            self.spectrum, self.spectrum, mode="full"
        )
        if norm:
            self.homogeneous_spec_corr = self.homogeneous_spec_corr / max(
                self.homogeneous_spec_corr
            )

        self.w0 = eV_to_angfreq(weg / 1000)

        if self.use_torch:
            self.homogeneous_spec_corr = np_to_torch(
                self.homogeneous_spec_corr, self.device
            )
    ...
    def poisson_diffusion(r, τ, δ, σ, p, ε=None):
        σ = eV_to_wn(
            σ / 1000
        )  # convert σ from meV to eV to cm^-1, to cancel out stage delay in cm
    ...
    def make_flat(self, α, p, ι_max, ι_min, add_δ=False, mode="Saturate"):
        ...
        self.flat = flat_mode(
            ζ=ζ, τ=Tau, α=α, ι_max=ι_max, ι_min=ι_min, p=p, add_δ=add_δ
        )
    ...
    def convolve(self, diffusion="wiener", norm=True, add_baseline=False):
        ...
        if self.use_torch:
            ...
            flat_convolve = signal.fftconvolve(
                d.flatten(), self.homogeneous_spec_corr, mode="same"
            )
    ...
    def make_interferogram(self, norm=True):
        if self.use_torch:
            self.I = torch.real(
                torch.fft.fftshift(torch.fft.fft(torch.fft.ifftshift(self.spec_corr)))
            )
        else:
    ...
    def downsample(
        self, symmetric=False, nonlinear_steps=False, nonlinear_args=[0, 1, 10]
    ):
        """
    ...
    def plot_spec_corr(
        ...
        for i, t in enumerate(taus):
            ...
            plt.plot(
                self.ζ,
                speccorr[idx1:idx2, :].mean(axis=0) + spacer * i,
                color=colors[i],
                label=("%1.0e - %1.0e ps") % (t[0], t[1]),
            )
        dress_fig(
            tight=not fixframe,
            xlabel="$\\zeta$ (meV)",
    ...
    def plot_inteferogram(
        ...
        for i, t in enumerate(taus):
            ...
            plt.plot(
                self.δ,
                self.I[idx1:idx2, :].mean(axis=0) + spacer * i,
                ms=2,
                marker=marker,
                color=self.colors[i],
                label=("%1.0e - %1.0e ps") % (t[0], t[1]),
...
def generate_pcfs_triplet_wiener(Γ, a1, a2, a3, o1, o2, y0, weg, g2_amp, σ_max, p, α):
    ...
    pcfs.make_spectrum(
        weg=weg,
        lineshape=threeLorentzian_oneFWHM,
        theta=[Γ, a1, a2, a3, o1, o2, y0],
        nbins=1501,
        emax=5,
    )
...
def generate_pcfs(
    ...
    pcfs.make_spectrum(
        weg=weg, lineshape=lineshape_function, theta=spectrum_theta, nbins=2001, emax=5
    )
...
if __name__ == "__main__":
    ...
    dot_name = "experimental_dot.pickle"  # perovskite
    ...
    pickle_in = open(dot_path + dot_name, "rb")
    ...
    lineshape_theta, lineshape_vars = make_theta_twoLorentzian_oneJ(
        param_path + "spectrum_params_CdSe.xls"
    )
    # lineshape_function = threeLorentzian_oneFWHM
    ...
    diffusion_theta, diffusion_vars = make_theta_diffusion(
        param_path + "diffusion_params_CK174_dot6.xls"
    )
    ...
    pcfs.make_spectrum(
        weg=2330,
        lineshape=lineshape_function,
        theta=lineshape_theta["init"],
        nbins=2001,
        emax=5,
    )
