...
def g2_function(
    x: np.ndarray, w0: float, w1: float, w2: float, w3: float, w4: float, dummy
) -> np.ndarray:
    return w0 + w1 * (
        np.exp(-np.abs(x + w4) / w3)
        + np.exp(-np.abs(x - w4) / w3)
        + w2 * np.exp(-np.abs(x) / w3)
    )
...
def g2_function_monoexp(
    ...
    """
    return w1 + w3 * (
        w0 * np.exp(-np.abs(x) / w4)
        + np.exp(-np.abs(x + w2) / w4)
        + np.exp(-np.abs(x - w2) / w4)
    )
...
def g2_function_biexp(
    ...
    return (
        ...
        * (
            w0 * np.exp(-np.abs(x) / w4)
            + np.exp(-np.abs(x + 1 * w2) / w4)
            + np.exp(-np.abs(x - 1 * w2) / w4)
            + np.exp(-np.abs(x + 2 * w2) / w4)
            + np.exp(-np.abs(x - 2 * w2) / w4)
            + np.exp(-np.abs(x + 3 * w2) / w4)
            + np.exp(-np.abs(x - 3 * w2) / w4)
        )
        + (1 - w3)
        * (
            w0 * np.exp(-np.abs(x) / w5)
            + np.exp(-np.abs(x + 1 * w2) / w5)
            + np.exp(-np.abs(x - 1 * w2) / w5)
            + np.exp(-np.abs(x + 2 * w2) / w5)
            + np.exp(-np.abs(x - 2 * w2) / w5)
            + np.exp(-np.abs(x + 3 * w2) / w5)
            + np.exp(-np.abs(x - 3 * w2) / w5)
        )
    ) + w1


def generate_gridded_spectra(
    num_points: int,
    timesteps: int = 1501,
    bounds: tuple = None,
    trange: tuple = (-1.5, 1.5),
    nexps: int = 1,
    ...
    if bounds == None:
        w0_points = np.linspace(
            0.0, 1.0, num_points, dtype=np.float32
        )  # r - center-to-side peak ratio
        w1_points = np.linspace(
            0.0, 0.075, num_points, dtype=np.float32
        )  # y0 - background constant
        w2_points = np.linspace(
            0.99, 1.01, num_points, dtype=np.float32
        )  # rr - laser repetition rate
        w3_points = np.linspace(
            0.0, 1.0, num_points, dtype=np.float32
        )  # a1 - amplitude of first lifetime
        w4_points = np.linspace(
            2e-2, 1.0e-1, num_points, dtype=np.float32
        )  # t1 - first lifetime (in µs)
        w5_points = np.linspace(
            1e-2, 0.5e-1, num_points, dtype=np.float32
        )  # t2 - second lifetime (in µs)

    else:
        w0_points = np.linspace(
            bounds[0][0], bounds[0][1], num_points, dtype=np.float32
        )
        w1_points = np.linspace(
            bounds[1][0], bounds[1][1], num_points, dtype=np.float32
        )
        w2_points = np.linspace(
            bounds[2][0], bounds[2][1], num_points, dtype=np.float32
        )
        w3_points = np.linspace(
            bounds[3][0], bounds[3][1], num_points, dtype=np.float32
        )
        w4_points = np.linspace(
            bounds[4][0], bounds[4][1], num_points, dtype=np.float32
        )
        if nexps == 2:
            w5_points = np.linspace(
                bounds[5][0], bounds[5][1], num_points, dtype=np.float32
            )
...
def create_dataset(
    ...
    spectra, params, t = generate_gridded_spectra(
...
def make_training_g2s(
    params, pcfs_function, randomize_lineshape=None, randomize_diffusion=None, logt=True
):
    """
...
def make_training_g2s_2d(
    params,
    pcfs_function,
    randomize_lineshape=None,
    randomize_diffusion=None,
    lag_precision=6,
    ...
    for i, p in enumerate(tqdm(params)):
        ...
        simPCFS = pcfs_function(
            w=p,
            lineshape_type=lineshape_type,
        ...
        # g2_array[:nstage, :] = simPCFS.g2.T
        g2_array = simPCFS.g2.T
        g2s.append(g2_array)

        # optical_delay_array[:nstage] = simPCFS.δ
        optical_delay_array = simPCFS.δ
        optical_delays.append(optical_delay_array)

        paramstack.append(p)
        spectra.append(simPCFS.spectrum)
        I_last.append(simPCFS.I[-1, :])
        nstages.append(nstage)

        dfs.append(simPCFS.df)
    ...
    return g2s, paramstack, spectra, ts, dfs, nstages, optical_delays, I_last, simPCFS
